...
- **🏥 Пошук пацієнта** — не готовий шаблон, а пошук наживо: поле "ПІБ або ІПН" (мін. 2 символи) шле запит на `/api/patients/search` (`service_role`, лише сервер — див. "Підключення до бази"), список результатів (до 20), клік на результат показує всі поля пацієнта і кнопку "Додати картку на полотно" — створює елемент "Список" зі стовпцями "Поле"/"Значення", по рядку на кожне з 28 полів (`lib/patient-fields.ts`)
...
### Імпорт / Експорт
- **💾 Зберегти JSON** — вивантажує весь проєкт: сторінки, елементи, вміст бібліотеки й стан усіх п'яти плаваючих панелей (позиція, розмір, прозорість, розгорнуті секції параметрів)
